...
from src.other.function import if_num, if_name, have_name, printf, get_word
        ...
        for word in in_words:
            ...
            # 判断为分隔符
                ...
                if w in kuo_cp.values() or w in kuo_cp.keys():
                    ...
                    # 右括号判断是否匹配并出栈
                        ...
                        self.flag = False
                        self.error = "第" + str(line) + "行的' " + w + " '无法匹配，无法通过编译，请检查代码正确性！"
                        return
            ...
            # 其他字符处理
                ...
                # 如果是变量名要判断是否已经存在
                    ...
                    self.flag = False
                    self.error = "第" + str(line) + "行的变量名' " + w + " '不可识别，无法通过编译，请检查代码正确性！"
                    return
        if kuo_list:
            # print("第" + str(kuo_list[0]['line']) + "行的' " + kuo_list[0]['kuo'] + " '无法匹配，无法通过编译，请检查代码正确性！")
            self.flag = False
            self.error = "第" + str(kuo_list[0]['line']) + "行的' " + kuo_list[0]['kuo'] + " '无法匹配，无法通过编译，请检查代码正确性！"
            return
